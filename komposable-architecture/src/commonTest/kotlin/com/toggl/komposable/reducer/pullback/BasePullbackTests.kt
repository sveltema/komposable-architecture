...
import com.toggl.komposable.common.FakeReducer
...
abstract class BasePullbackTests {
    abstract val localReducer: FakeReducer<LocalTestState, LocalTestAction>
    ...
    fun `local reducer should be called when mapToLocalAction returns actions`() = runTest {
        ...
        assertTrue { localReducer.invocations.contains(LocalTestState(1) to action.action) }
    }
    ...
    fun `local reducer should not be called when mapToLocalAction returns null`() = runTest {
        ...
        pulledBackReducer.testReduceNoOp(globalState, action)
